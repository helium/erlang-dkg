...
-record(serialized_vss, {
            ...
            received_commitment = false :: boolean(),
            readies = #{} :: readies(),
            commitment :: dkg_commitment:serialized_commitment()
         }).
...
input(VSS = #vss{session=Session={Dealer,_}, id=Id, u=U, u2=U2, t=T, n=N}, Secret) when Dealer == Id ->
    ...
    NewVSS = VSS#vss{commitment=Commitment, received_commitment=true},
    {NewVSS, {send, Msgs}};
input(VSS, _Secret) ->
    {VSS, ok}.
...
handle_msg(VSS=#vss{n=N, u=U, id=Id, session=Session, sent_echo=false}, Sender, {send, {Session = {Sender, _}, SerializedCommitmentMatrix0, SA}}) ->
    ...
    case dkg_commitmentmatrix:verify_poly(VSS#vss.u2, CommitmentMatrix0, Id, A) of
        true ->
            %% always update the commitment matrix if we get it straight from the dealer
            Commitment = dkg_commitment:set_matrix(VSS#vss.commitment, CommitmentMatrix0),
            Msgs = lists:map(fun(Node) ->
                                     {unicast, Node, {echo, {Session, SerializedCommitmentMatrix0, erlang_pbc:element_to_binary(dkg_polynomial:evaluate(A, Node))}}}
                             end, dkg_util:allnodes(N)),
            {VSS#vss{sent_echo=true, commitment=Commitment, received_commitment=true}, {send, Msgs}};
        false ->
            {VSS, ok}
    end;
...
handle_msg(VSS=#vss{id=Id, n=N, t=T, session=Session}, Sender, {echo, {Session, SerializedCommitmentMatrix0, SA}}) ->
    ...
    case dkg_commitmentmatrix:verify_point(VSS#vss.u2, CommitmentMatrix0, Sender, Id, A) of
        true ->
            Commitment = case VSS#vss.received_commitment of
                             true -> VSS#vss.commitment;
                             false -> dkg_commitment:set_matrix(VSS#vss.commitment, CommitmentMatrix0)
                         end,
                ...
                {true, NewCommitment} ->
                    case dkg_commitment:num_echoes(NewCommitment) == ceil((N+T+1)/2) andalso
                        ...
                        true ->
                            ...
                            NewVSS = VSS#vss{commitment=NewCommitment, sent_ready=true, received_commitment=true},
                            {NewVSS, {send, Msgs}};
                        false ->
                            NewVSS = VSS#vss{ commitment=NewCommitment, received_commitment=true},
                            {NewVSS, ok}
                    end;
                {false, OldCommitment} ->
                    {VSS#vss{commitment=OldCommitment, received_commitment=true}, ok}
            end;
        false ->
            {VSS, ok}
    end;
                ...
                {true, NewCommitment} ->
                    case dkg_commitment:num_readies(NewCommitment) == (T+1) andalso
                        ...
                        true ->
                            ...
                            NewVSS = store_ready(Sender, ReadyMsg, VSS#vss{commitment=NewCommitment, received_commitment=true}),
                            {NewVSS, {send, Msgs}};
                        false ->
                                ...
                                true->
                                    case VSS#vss.done of
                                        ...
                                        false ->
                                            ...
                                            NewVSS = VSS#vss{readies=maps:put(Sender, ReadyMsg, Readies), commitment=NewCommitment, received_commitment=true},
                                            %% clear the readies out of our state and hand them off to the DKG
                                            {NewVSS#vss{readies=#{}, done=true}, {result, {Session, Commitment, SubShare, NewVSS#vss.readies}}}
                                    end;
                                false ->
                                    NewVSS = store_ready(Sender, ReadyMsg, VSS#vss{commitment=NewCommitment, received_commitment=true}),
                                    {NewVSS, ok}
...
-spec commitment(vss()) -> dkg_commitment:commitment().
commitment(VSS) ->
    VSS#vss.commitment.
